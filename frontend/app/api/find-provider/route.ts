...
// Proxy for the Python backend's checklist "Find a provider" action (see
// backend/explore-mcp/tools/find_provider.py and server.py's
...
// label is read server-side from the project the caller is verified to
// belong to, never trusted from the request body, so a forged request can't
// ask this route to map/search text for a project the caller isn't on.
//
...
// HTTP 200, so the checklist item panel can render a clean fallback message
...
export async function POST(req: Request) {
  ...
  try {
    const res = await fetch(`${EXPLORE_API_URL}/api/find-provider`, {
      ...
      body: JSON.stringify({ item_text: item.label }),
    });
